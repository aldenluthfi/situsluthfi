...
const tagIconMap: Record<string, React.ElementType> = {
  ...
  Politics: (props) => <IconChessKing className="size-6" stroke={1.5} {...props} />,
    ...
    <div className="flex flex-col min-h-screen items-center overflow-clip">
      ...
      <div className="w-full max-w-3xl px-12 flex flex-col gap-6 pb-24">
        {loading
          ...
          : data.map((item) => (
            <Link key={item.id} to={`/writings/${item.id}`} className="hover:motion-scale-out-105 motion-scale-in-105 motion-ease-spring-snappy motion-duration-300">
              <Card key={item.id}>
                <CardHeader>
